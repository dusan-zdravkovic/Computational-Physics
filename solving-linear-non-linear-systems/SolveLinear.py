...
def PartialPivot(A_in, v_in):
    ...
    for m in range(N):
        # Partial pivot
        # Keep track of greatest abs of diagonal element and its row
        m_max = m
        max_val = abs(A[m, m])

        for m_lower in range(m + 1, N):
            curr = abs(A[m_lower, m])
            if curr > max_val:
                max_val = curr
                m_max = m_lower
        # check if swap is needed
        if m != m_max:
            # Swap the rows for partial pivot
            A[[m, m_max]] = A[[m_max, m]]
            # Remember to swap the v_in numbers too
            v[[m, m_max]] = v[[m_max, m]]
